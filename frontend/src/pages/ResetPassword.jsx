...
export default function ResetPassword() {
  ...
  const submit = async (e) => {
    e.preventDefault()
    if (!token) {
      setError(t('Reset token is missing.'))
      return
    }
    ...
    try {
      ...
      setError(err?.response?.data?.detail || err?.response?.data?.message || err?.message || t('Password reset failed.'))
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-black flex items-center justify-center p-6">
      <div className="w-full max-w-md p-8 rounded-3xl bg-white/[0.03] border border-white/10">
        ...
        <h1 className="text-2xl font-black text-white mb-2">{t('Reset password')}</h1>
        <p className="text-white/55">{t('Set a new secure password for your account.')}</p>
        <ul className="mt-3 mb-6 space-y-1 text-sm text-white/45">
          <li>{t('Use at least 8 characters.')}</li>
          <li>{t('Mix letters, numbers, and symbols.')}</li>
          <li>{t('Avoid reusing an old password.')}</li>
        </ul>
        <form onSubmit={submit} className="space-y-4">
          ...
          <button disabled={loading} className="w-full py-3 rounded-xl bg-white text-black font-bold inline-flex items-center justify-center gap-2 disabled:opacity-50">
            {loading ? t('Updating...') : <>{t('Reset password')} <ArrowRight className="w-4 h-4" /></>}
          </button>
        </form>
