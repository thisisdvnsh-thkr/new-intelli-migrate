...
export default function ForgotPassword() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setError(err?.response?.data?.detail || t('Could not send reset link.'))
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-black flex items-center justify-center p-6">
      <div className="w-full max-w-md p-8 rounded-3xl bg-white/[0.03] border border-white/10">
        ...
        <h1 className="text-2xl font-black text-white mb-2">{t('Forgot password')}</h1>
        <p className="text-white/55 mb-6">{t('Enter your account email to receive a secure reset link.')}</p>
        <form onSubmit={submit} className="space-y-4">
          {error && <div className="p-3 rounded-xl bg-red-500/10 border border-red-500/25 text-red-300 text-sm">{error}</div>}
          {done && <div className="p-3 rounded-xl bg-green-500/10 border border-green-500/25 text-green-300 text-sm">{t('Reset link sent if this email exists.')}</div>}
          ...
          <button disabled={loading} className="w-full py-3 rounded-xl bg-white text-black font-bold inline-flex items-center justify-center gap-2 disabled:opacity-50">
            {loading ? t('Sending...') : <>{t('Send reset link')} <ArrowRight className="w-4 h-4" /></>}
          </button>
        </form>
        <p className="mt-6 text-sm text-white/50">{t('Back to')} <Link className="text-blue-300" to="/login">{t('Login')}</Link></p>
      </div>
    </div>
  )
}
